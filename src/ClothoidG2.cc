...
int G2solve2arc::build( real_type const x0, real_type const y0, real_type const theta0,
    real_type const kappa0, real_type const x1, real_type const y1, real_type const theta1,
...
{
    UTILS_ASSERT( miter > 0 && miter <= 1000,
...
void G2solve2arc::evalA( real_type const alpha, real_type const L, real_type& A ) const
{
...
void G2solve2arc::evalA(
    real_type const alpha, real_type const L, real_type& A, real_type& A_1, real_type& A_2 ) const
{
    real_type const K{ m_k0 + m_k1 };
    real_type const aK{ alpha * m_DeltaK };
    A = alpha * ( L * ( aK - K ) + 2 * m_DeltaTheta );
...
void G2solve2arc::evalG( real_type const alpha, real_type const L, real_type const th,
    real_type const k, real_type G[2] ) const
{
    real_type A, X, Y;
    evalA( alpha, L, A );
...
void G2solve2arc::evalG( real_type const alpha, real_type const L, real_type const th,
    real_type const k, real_type G[2], real_type G_1[2], real_type G_2[2] ) const
{
    ...
    do {
      ...
      X[1] -= d[1];
        ...
        do {
            ...
            evalF( XX, FF );
            solver.solve( FF, dd );
            step_found = hypot( dd[0], dd[1] ) <= ( 1 - tau / 2 ) * nd + 1e-6 && XX[0] > 0 &&
                XX[0] < 1 && XX[1] > 0;
...
void G2solve2arc::build_solution( real_type const alpha, real_type const L )
{
...
int G2solveCLC::build( real_type const x0, real_type const y0, real_type const theta0,
    real_type const kappa0, real_type const x1, real_type const y1, real_type const theta1,
    real_type const kappa1 )
{
    m_x0 = x0;
...
int G2solveCLC::solve()
{
    ...
    do {
        ...
        real_type const dF{ D0 * m_k1 * ( X0[2] - 2 * X0[1] + X0[0] ) -
            D1 * m_k0 * ( X1[2] - 2 * X1[1] + X1[0] ) - m_k0 * m_k1 * cos( thM ) + m_k1 * Y0[0] -
            m_k0 * Y1[0] };
...
bool G2solveCLC::build_solution( real_type const sM, real_type const thM )
{
...
int G2solve3arc::build( real_type const x0, real_type const y0, real_type const theta0,
    real_type const kappa0, real_type const x1, real_type const y1, real_type const theta1,
    real_type const kappa1, real_type Dmax, real_type dmax )
{
    try {
        // save data
        m_x0 = x0;
...
int G2solve3arc::build_fixed_length( real_type const s0, real_type const x0, real_type const y0,
    real_type const theta0, real_type const kappa0, real_type const s1, real_type const x1,
    real_type const y1, real_type const theta1, real_type const kappa1 )
{
    try {
        // save data
        m_x0 = x0;
...
int G2solve3arc::solve( real_type const sM_guess, real_type const thM_guess )
{
    real_type X[2];
    X[0] = sM_guess;
    X[1] = thM_guess;
...
void G2solve3arc::build_solution( real_type const sM, real_type const thM )
{
    ...
    */
    ...
    m_SM.build( m_x0 + C * dx - S * dy, m_y0 + C * dy + S * dx, thM + m_phi, KM, dKM, 2 * LM );
...
real_type G2solve3arc::curvature_min_max( real_type& kMin, real_type& kMax ) const
{
...
// offset curve
void G2solve3arc::eval_ISO( real_type s, real_type const offs, real_type& x, real_type& y ) const
{
    if ( s < m_S0.length() ) {
...
void ClothoidSplineG2::guess(
    real_type theta_guess[], real_type theta_min[], real_type theta_max[] ) const
{
...
void ClothoidSplineG2::build( real_type const xvec[], real_type const yvec[], integer const n )
{
    m_npts = n;
...
integer ClothoidSplineG2::numConstraints() const
{
    switch ( m_tt ) {
    case TargetType::P1:
    case TargetType::P2:
...
bool ClothoidSplineG2::objective( real_type const theta[], real_type& f ) const
{
    ...
    case TargetType::P9:
        ...
        for ( integer j{ 0 }; j < ne; ++j ) {
            ...
            f += ( k4 + dk2 +
                     ( 2 * k3 * dkur +
                         ( 2 * k2 * dk2 + ( dk3 * ( kur + dkur * Len / 5 ) ) * Len ) * Len ) *
                         Len ) *
                Len;
        }
        break;
...
bool ClothoidSplineG2::gradient( real_type const theta[], real_type g[] ) const
{
    ...
    case TargetType::P4:
        ...
        cR.build_G1_D(
            m_x[ne1], m_y[ne1], theta[ne1], m_x[ne], m_y[ne], theta[ne], LR_D, kR_D, dkR_D );
        {
    ...
    case TargetType::P6:
        for ( integer j{ 0 }; j < ne; ++j ) {
            ...
            c.build_G1_D(
                m_x[j], m_y[j], theta[j], m_x[j + 1], m_y[j + 1], theta[j + 1], L_D, k_D, dk_D );
            g[j] += L_D[0];
            g[j + 1] += L_D[1];
        }
        break;
    case TargetType::P7:
        for ( integer j{ 0 }; j < ne; ++j ) {
            ...
            g[j] += 2 * ( dkur * dk_D[0] * L3 ) / 3 + ( dk2 * L2 * L_D[0] ) + dk_D[0] * L2 * kur +
                2 * dkur * Len * L_D[0] * kur + dkur * L2 * k_D[0] + L_D[0] * k2 +
                2 * Len * kur * k_D[0];
            g[j + 1] += 2 * ( dkur * dk_D[1] * L3 ) / 3 + ( dk2 * L2 * L_D[1] ) +
                dk_D[1] * L2 * kur + 2 * dkur * Len * L_D[1] * kur + dkur * L2 * k_D[1] +
                L_D[1] * k2 + 2 * Len * kur * k_D[1];
        }
        break;
    case TargetType::P8:
        for ( integer j{ 0 }; j < ne; ++j ) {
    ...
    case TargetType::P9:
        for ( integer j{ 0 }; j < ne; ++j ) {
            ...
            real_type const A{
                ( ( ( dkL + 4 * kur ) * dkL + 6 * k2 ) * dkL + 4 * k3 ) * dkL + dk2 + k2 * k2 };
            real_type const B{
                ( ( ( ( 3 * kur + 0.8 * dkL ) * dkL + 4 * k2 ) * dkL + 2 * k3 ) * Len + 2 * dkur ) *
                Len };
...
integer ClothoidSplineG2::jacobian_nnz() const
{
...
bool ClothoidSplineG2::jacobian_pattern( integer ii[], integer jj[] ) const
{
