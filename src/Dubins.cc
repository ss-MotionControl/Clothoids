...
Dubins::Dubins( real_type x0, real_type y0, real_type theta0, real_type x1, real_type y1,
    real_type theta1, real_type k_max, string_view const name )
    : BaseCurve( name )
{
...
//!
bool Dubins_build( real_type const x0, real_type const y0, real_type const theta0,
    real_type const x3, real_type const y3, real_type const theta3, real_type const k_max,
    DubinsType& type, real_type& L1, real_type& L2, real_type& L3, real_type grad[2] )
{
    using std::abs;
...
#define CHECK_DUBINS( TYPE )      \
    ...
    auto LRL = [&]() -> void {
        Quadratic const Q(
            cab - 1 + dd2 + d * sasb, 4 * ( dcb + sab ), 3 + dd2 + dsa - 3 * ( dsb + cab ) );
        real_type Z[2];
...
bool Dubins::build( real_type const x0, real_type const y0, real_type const theta0,

    real_type const x3, real_type const y3, real_type const theta3,

    real_type const k_max )
{
    real_type L1, L2, L3, grad[2];
    bool const ok{
        Dubins_build( x0, y0, theta0, x3, y3, theta3, k_max, m_solution_type, L1, L2, L3, grad ) };
...
bool Dubins::collision_ISO( real_type const offs, Dubins const& B, real_type const offs_B ) const
{
    return m_C0.collision_ISO( offs, B.m_C0, offs_B ) ||
        m_C0.collision_ISO( offs, B.m_C1, offs_B ) || m_C0.collision_ISO( offs, B.m_C2, offs_B ) ||
...
void Dubins::intersect_ISO(
    real_type const offs, Dubins const& B, real_type const offs_B, IntersectList& ilist ) const
{
    IntersectList ilist00, ilist01, ilist02, ilist10, ilist11, ilist12, ilist20, ilist21, ilist22;
...
integer Dubins::closest_point_ISO( real_type const qx, real_type const qy, real_type& x,
...
integer Dubins::closest_point_ISO( real_type const qx, real_type const qy, real_type const offs,
...
integer Dubins::get_range_angles_begin( real_type const x0, real_type const y0, real_type const x1,
    real_type const y1, real_type const theta1, real_type const k_max, real_type angles[] ) const
{
    // setup in standard form
    ...
    { // case CCC
        ...
        for ( integer i{ 0 }; i < 2; ++i ) {
            ...
            real_type const C{ ( t14 * t7 - 16 ) * sb * t5 - 2 * ( d * d ) * t7 + 12 * t2 +
                4 * t7 * ( t2 + 1 ) + 52 };
...
integer Dubins::get_range_angles_end( real_type const x0, real_type const y0,
    real_type const theta0, real_type const x1, real_type const y1, real_type const k_max,
    real_type angles[] ) const
{
    ...
    { // case CCC
        ...
        for ( integer i{ 0 }; i < 2; ++i ) {
            ...
            real_type const C{ -2 * ( d * d ) * t5 + ( t14 * t5 + 16 ) * sa * t8 + 12 * t2 +
                4 * t5 * ( t2 + 1 ) + 52 };
