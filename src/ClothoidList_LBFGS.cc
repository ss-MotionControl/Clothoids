...
namespace G2lib
{
    ...
    std::function<real_type( ClothoidList const & lst )> const & target )
  {
    ...
    auto TARGET_old = [&n, &X0, &Y0, &nx, &ny, &target, &w_h, &theta_init, &theta_end](
                        Vector const & offs,
                        Vector *       grad ) -> real_type
    {
      ...
      if ( grad != nullptr )
      {
        for ( integer i{ 0 }; i < n; ++i )
        {
          ...
          ok              = tmp1.build_G2( n, X.data(), Y.data(), theta_init, theta_end );
          real_type vp{ target( tmp1 ) };
          X.coeffRef( i ) = x_save - w_h * nx_save;
          Y.coeffRef( i ) = y_save - w_h * ny_save;
          ok              = tmp2.build_G2( n, X.data(), Y.data(), theta_init, theta_end );
  ...
  bool ClothoidList::build_G2_cyclic_with_target(
    integer const                                                n,
    real_type const                                              x[],
    real_type const                                              y[],
    real_type const                                              theta[],
    ...
    std::function<real_type( ClothoidList const & lst )> const & target )
  {
    ...
    // Utils::WeakWolfeLineSearch<real_type> line_search;
    Utils::ArmijoLineSearch<real_type> line_search;
    minimizer.minimize( offs0, TARGET, line_search );

    bool ok{ minimizer.status() == Utils::LBFGS_minimizer<real_type>::Status::CONVERGED };
