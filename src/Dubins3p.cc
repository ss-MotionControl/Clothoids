...
Dubins3pBuildType string_to_Dubins3pBuildType( string_view const str )
{
    map<string_view, Dubins3pBuildType> const str_to_type{
        ...
        { "pattern_search_bracket", Dubins3pBuildType::PATTERN_SEARCH_WITH_ALGO_BRACKET },
        { "trichotomy", Dubins3pBuildType::PATTERN_TRICHOTOMY },
        { "trichotomy_bracket", Dubins3pBuildType::PATTERN_TRICHOTOMY_WITH_ALGO_BRACKET },
        { "pattern_trichotomy", Dubins3pBuildType::PATTERN_TRICHOTOMY },
        { "pattern_trichotomy_bracket", Dubins3pBuildType::PATTERN_TRICHOTOMY_WITH_ALGO_BRACKET },
...
bool Dubins3p::build( real_type const xi, real_type const yi, real_type const thetai,
    real_type const xm, real_type const ym, real_type const xf, real_type const yf,
    real_type const thetaf, real_type const k_max, Dubins3pBuildType const method )
{
    switch ( method ) {
    ...
    case Dubins3pBuildType::PATTERN_SEARCH:
        return build_pattern_search(
            xi, yi, thetai, xm, ym, xf, yf, thetaf, k_max, m_tolerance, false, false );
    case Dubins3pBuildType::PATTERN_TRICHOTOMY:
        return build_pattern_search(
            xi, yi, thetai, xm, ym, xf, yf, thetaf, k_max, m_tolerance, true, false );
    case Dubins3pBuildType::PATTERN_SEARCH_WITH_ALGO_BRACKET:
        return build_pattern_search(
            xi, yi, thetai, xm, ym, xf, yf, thetaf, k_max, m_tolerance, false, true );
    case Dubins3pBuildType::PATTERN_TRICHOTOMY_WITH_ALGO_BRACKET:
        return build_pattern_search(
            xi, yi, thetai, xm, ym, xf, yf, thetaf, k_max, m_tolerance, true, true );
...
bool Dubins3p::build_sample( real_type const xi, real_type const yi, real_type const thetai,
    real_type const xm, real_type const ym, real_type const xf, real_type const yf,
    real_type const thetaf, real_type const k_max )
{
    m_evaluation = 0;
    real_type thetam{ 0 };
...
{
    UTILS_ASSERT( 180 * ang > Utils::m_pi && 3 * ang <= Utils::m_2pi,
...
{
    UTILS_ASSERT( npts >= 4 && npts <= 36000000,
...
{
    UTILS_ASSERT( max_eval > 0 && max_eval < 1000000,
...
void Dubins3p::eval(
    real_type s, real_type& theta, real_type& kappa, real_type& x, real_type& y ) const
{
...
{
    Dubins const TMP{ m_Dubins0 };
    m_Dubins0.copy( m_Dubins1 );
    m_Dubins1.copy( TMP );
    m_Dubins0.reverse();
    m_Dubins1.reverse();
}
...
void Dubins3p::scale( real_type const scl )
{
    m_Dubins0.scale( scl );
    m_Dubins1.scale( scl );
...
void Dubins3p::change_origin( real_type const newx0, real_type const newy0 )
{
...
void Dubins3p::bbox( real_type& xmin, real_type& ymin, real_type& xmax, real_type& ymax ) const
{
...
bool Dubins3p::collision( Dubins3p const& B ) const
{
    return m_Dubins0.collision( B.m_Dubins0 ) || m_Dubins0.collision( B.m_Dubins1 ) ||
        m_Dubins1.collision( B.m_Dubins0 ) || m_Dubins1.collision( B.m_Dubins1 );
}

bool Dubins3p::collision_ISO(
    real_type const offs, Dubins3p const& B, real_type const offs_B ) const
{
...
bool Dubins3p::collision( BaseCurve const* pC ) const
{
...
void Dubins3p::intersect( Dubins3p const& B, IntersectList& ilist ) const
{
    ...
    ilist.reserve( ilist.size() +
        ...
        ilist00.size() + ilist01.size() +

        ilist10.size() + ilist11.size() );
...
void Dubins3p::intersect_ISO(
    real_type const offs, Dubins3p const& B, real_type const offs_B, IntersectList& ilist ) const
{
    IntersectList ilist00, ilist01, ilist10, ilist11;
...
integer Dubins3p::closest_point_ISO( real_type const qx, real_type const qy, real_type& x,
...
void Dubins3p::intersect( BaseCurve const* pC, IntersectList& ilist ) const
{
    if ( pC->type() == CurveType::DUBINS3P ) {
        Dubins3p const& C = *dynamic_cast<Dubins3p const*>( pC );
...
integer Dubins3p::closest_point_ISO( real_type const qx, real_type const qy, real_type const offs,
...
string Dubins3p::solution_type_string() const
{
...
integer Dubins3p::get_range_angles( real_type const xi, real_type const yi, real_type const thetai,
    real_type const xm, real_type const ym, real_type const xf, real_type const yf,
    real_type const thetaf, real_type const k_max, real_type angles[] ) const
{
    integer npts{ 0 };
