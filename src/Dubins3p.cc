...
namespace G2lib
{
  ...
  */
    ...
    Dubins3pBuildType const method )
  {
    switch ( method )
    {
      case Dubins3pBuildType::SAMPLE_ONE_DEGREE: return build_sample( xi, yi, thetai, xm, ym, xf, yf, thetaf, k_max );
    ...
    case Dubins3pBuildType::PATTERN_TRICHOTOMY_WITH_ALGO_BRACKET:
      ...
      case Dubins3pBuildType::ELLIPSE: return build_ellipse( xi, yi, thetai, xm, ym, xf, yf, thetaf, k_max );
      case Dubins3pBuildType::POLYNOMIAL_SYSTEM: break;
    }
    return false;
  }

  bool Dubins3p::build_sample(
    real_type const xi,
    real_type const yi,
    real_type const thetai,
    real_type const xm,
...
#define DUBINS_SELECT_EVAL(FUN,...)    \
  if ( s < m_Dubins0.length() ) { m_Dubins0.FUN( s, __VA_ARGS__ ); } \
  else                                                               \
  {                                                                  \
    s -= m_Dubins0.length();           \
  ...
  bool Dubins3p::collision_ISO( real_type const offs, Dubins3p const & B, real_type const offs_B ) const
  {
    return m_Dubins0.collision_ISO( offs, B.m_Dubins0, offs_B ) ||
           m_Dubins0.collision_ISO( offs, B.m_Dubins1, offs_B ) ||

           m_Dubins1.collision_ISO( offs, B.m_Dubins0, offs_B ) || m_Dubins1.collision_ISO( offs, B.m_Dubins1, offs_B );
  }
  ...
  bool Dubins3p::collision_ISO( real_type const offs, BaseCurve const * pC, real_type const offs_C ) const
  {
  ...
  integer Dubins3p::closest_point_ISO(
    real_type const qx,
    real_type const qy,
    real_type     & x,
    real_type     & y,
  ...
  integer Dubins3p::get_range_angles(
    real_type const xi,
    real_type const yi,
    real_type const thetai,
    real_type const xm,
