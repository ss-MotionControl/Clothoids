...
namespace G2lib
{
  ...
  real_type ClothoidCurve::m_tolerance = 1e-10;  // 1 degree
    ...
    real_type const tol )
  {
    ...
    m_aabb_triangles.clear();
    G2derivative G;
    bool         ok = m_CD.build_G1( x0, y0, theta0, x1, y1, theta1, G, tol );
    m_L             = G.L;
    return ok;
  }
    ...
    real_type const tol )
  {
    m_aabb_done = false;
    m_aabb_triangles.clear();
    bool ok = m_CD.build_G1_D( x0, y0, theta0, x1, y1, theta1, G, tol );
    m_L     = G.L;
    return ok;
  }
    ...
    real_type const tol )
  {
    m_aabb_done = false;
    m_aabb_triangles.clear();
    bool ok = m_CD.build_G1_DD( x0, y0, theta0, x1, y1, theta1, G, tol );
    m_L     = G.L;
    return ok;
  }
  ...
  bool ClothoidCurve::build_forward(
    real_type const x0,
    real_type const y0,
    real_type const theta0,
  ...
  real_type ClothoidCurve::length_ISO( real_type offs ) const
  {
    real_type const & k0 = m_CD.m_kappa0;
    real_type const & dk = m_CD.m_dk;

    // f(s) = 1 - offs * k(s)
    auto f = [&]( real_type s ) -> real_type { return 1 - offs * ( k0 + dk * s ); };

    // se non cambia segno → formula chiusa
    if ( f( 0 ) * f( m_L ) > 0 ) return m_L * f( m_L / 2 );

    // punto di annullamento (cuspide)
    real_type s0 = f( 0 ) / ( offs * dk );

    // integrazione a tratti con valore assoluto
    auto integral = [&]( real_type s1, real_type s2 ) -> real_type
    {
      real_type ds = s2 - s1;
      return ds - offs * ( k0 * ds + 0.5 * dk * ( s2 * s2 - s1 * s1 ) );
    };

    real_type L1 = std::abs( integral( 0, s0 ) );
    real_type L2 = std::abs( integral( s0, m_L ) );
  ...
  void ClothoidCurve::bb_triangles_internal_ISO(
    real_type const      offs,
  ...
  void ClothoidCurve::bb_triangles_ISO(
    real_type const      offs,
    vector<Triangle2D> & tvec,
    real_type const      max_angle,
    real_type const      max_size,
  ...
  void ClothoidCurve::build_AABBtree_ISO( real_type const offs, real_type const max_angle, real_type const max_size )
    const
  {
  ...
  bool ClothoidCurve::collision_ISO( real_type offs, ClothoidCurve const & C, real_type offs_C ) const
  {
    ...
    G2LIB_DEBUG_TOC;
    G2LIB_DEBUG_MESSAGE( "ClothoidCurve collision_ISO: collisions elapsed {}ms\n", tictoc.elapsed_ms() );
  ...
  bool ClothoidCurve::approximate_collision_ISO(
    ...
    real_type     const   max_angle,
    real_type const       max_size ) const
  {
  ...
  bool ClothoidCurve::collision_ISO( real_type const offs, BaseCurve const * pC, real_type const offs_C ) const
  {
    if ( pC->type() == CurveType::CLOTHOID )
    {
      ClothoidCurve const & C = *static_cast<ClothoidCurve const *>(pC);
  ...
  void ClothoidCurve::intersect_ISO( real_type offs, ClothoidCurve const & C, real_type offs_C, IntersectList & ilist )
    const
  {
    real_type ss1, ss2;
    G2LIB_DEBUG_TICTOC;
    ...
    if ( intersect_with_AABBtree )
    {
      ...
      G2LIB_DEBUG_TOC;
      G2LIB_DEBUG_MESSAGE( "ClothoidCurve intersect_ISO: build_AABBtree_ISO elapsed {}ms\n", tictoc.elapsed_ms() );
    ...
    real_type & dst ) const
  {
    ...
    for ( integer iter{ 0 }; iter < m_max_iter; ++iter )
    {
      ...
      {
        // force one more iteration to improve accuracy
    ...
    real_type & DST ) const
  {
    ...
    if ( m_aabb_tree.num_tree_nodes() > G2LIB_AABB_MIN_NODES && intersect_with_AABBtree )
    {
      ...
      G2LIB_DEBUG_MESSAGE( "ClothoidCurve::closest_point_internal minimize elapsed {}ms\n", tictoc.elapsed_ms() );
    }
    else
    {
      G2LIB_DEBUG_TIC;
  ...
  integer ClothoidCurve::closest_point_ISO(
    real_type const qx,
    real_type const qy,
    real_type const offs,
    real_type     & x,
  ...
  real_type ClothoidCurve::integral_jerk2() const
  {
    ...
    return ( ( ( ( t4 / 5 * m_CD.m_dk + t3 * m_CD.m_kappa0 ) * m_CD.m_dk + ( 1 + 2 * t2 ) * k2 ) * m_CD.m_dk +
               2 * t1 * k3 ) *
               m_CD.m_dk +
             k4 ) *
           m_L;
  ...
  real_type ClothoidCurve::integral_snap2() const
  {
    ...
    return ( ( t7 / 7 ) * dk6 + dk5 * m_CD.m_kappa0 * t6 + 3 * dk4 * k2 * t5 + 5 * dk3 * k3 * t4 + 5 * dk2 * k4 * t3 +
             3 * dk3 * t3 + 3 * m_CD.m_dk * k5 * t2 + 9 * dk2 * m_CD.m_kappa0 * t2 + k6 + 9 * k2 * m_CD.m_dk ) *
           m_L;
  }
