...
ClothoidCurve::ClothoidCurve( string_view const name ) : BaseCurve( name )
{
    m_CD.m_x0 = 0;
    m_CD.m_y0 = 0;
    m_CD.m_theta0 = 0;
...
ClothoidCurve::ClothoidCurve( real_type const x0, real_type const y0, real_type const theta0,
    real_type const k, real_type const dk, real_type const L, string_view const name )
    : BaseCurve( name )
{
    m_CD.m_x0 = x0;
    m_CD.m_y0 = y0;
...
ClothoidCurve::ClothoidCurve( real_type const P0[], real_type const theta0, real_type const P1[],
    real_type const theta1, string const& name )
    : BaseCurve( name )
{
...
void ClothoidCurve::copy( ClothoidCurve const& c )
{
    m_CD = c.m_CD;
    m_L = c.m_L;
    m_aabb_done = false;
...
void ClothoidCurve::build( LineSegment const& LS )
{
    m_CD.m_x0 = LS.m_x0;
    m_CD.m_y0 = LS.m_y0;
...
void ClothoidCurve::build( CircleArc const& C )
{
    m_CD.m_x0 = C.m_x0;
    m_CD.m_y0 = C.m_y0;
...
void ClothoidCurve::build(
    real_type x0, real_type y0, real_type theta0, real_type k, real_type dk, real_type L )
{
    UTILS_ASSERT( L > 0,
...
int ClothoidCurve::build_G1( real_type const x0, real_type const y0, real_type const theta0,
    real_type const x1, real_type const y1, real_type const theta1, real_type const tol )
{
    m_aabb_done = false;
    m_aabb_triangles.clear();
...
int ClothoidCurve::build_G1_D( real_type const x0, real_type const y0, real_type const theta0,
    real_type const x1, real_type const y1, real_type const theta1, real_type L_D[2],
    real_type k_D[2], real_type dk_D[2], real_type const tol )
{
    m_aabb_done = false;
    m_aabb_triangles.clear();
...
bool ClothoidCurve::build_forward( real_type const x0, real_type const y0, real_type const theta0,
    real_type const kappa0, real_type const x1, real_type const y1, real_type const tol )
{
    m_aabb_done = false;
    m_aabb_triangles.clear();
...
ClothoidCurve::ClothoidCurve( LineSegment const& LS ) : BaseCurve( LS.name() )
{
    m_CD.m_x0 = LS.m_x0;
    m_CD.m_y0 = LS.m_y0;
    m_CD.m_theta0 = LS.m_theta0;
...
ClothoidCurve::ClothoidCurve( CircleArc const& C ) : BaseCurve( C.name() )
{
    m_CD.m_x0 = C.m_x0;
    m_CD.m_y0 = C.m_y0;
    m_CD.m_theta0 = C.m_theta0;
...
ClothoidCurve::ClothoidCurve( BaseCurve const* pC ) : ClothoidCurve( pC->name() )
{
...
void ClothoidCurve::optimized_sample_internal_ISO( real_type const s_begin, real_type const s_end,
    real_type const offs, real_type const ds, real_type const max_angle,
    vector<real_type>& s ) const
{
    real_type ss = s_begin;
    real_type thh = theta( s_begin );
    for ( integer npts = 0; ss < s_end; ++npts ) {
        UTILS_ASSERT0( npts < 100000000,
...
void ClothoidCurve::optimized_sample_ISO( real_type const offs, integer const npts,
    real_type const max_angle, vector<real_type>& s ) const
{
    s.clear();
    s.reserve( static_cast<size_t>( npts ) );
    s.emplace_back( 0 );

    real_type const ds = m_L / npts;
...
void ClothoidCurve::bb_triangles_internal_ISO( real_type const offs, vector<Triangle2D>& tvec,
    real_type const s_begin, real_type const s_end, real_type const max_angle,
...
void ClothoidCurve::bb_triangles_ISO( real_type const offs, vector<Triangle2D>& tvec,
...
void ClothoidCurve::build_AABBtree_ISO(
...
{
    G2LIB_DEBUG_MESSAGE( "ClothoidCurve::collision_ISO( offs={}, C, offs_C={}\n", offs, offs_C );
    ...
    for ( const auto& [fst, snd] : intersectList ) {
        ...
        for ( integer j : snd ) {
            UTILS_ASSERT_DEBUG( j >= 0 && j < static_cast<integer>( C.m_aabb_triangles.size() ),
...
//!
bool ClothoidCurve::approximate_collision_ISO( real_type offs, ClothoidCurve const& C,
    real_type const offs_C, real_type const max_angle, real_type const max_size ) const
{
...
bool ClothoidCurve::collision( BaseCurve const* pC ) const
{
...
bool ClothoidCurve::collision_ISO(
...
bool ClothoidCurve::aabb_intersect_ISO( Triangle2D const& T1, real_type const offs,
    ClothoidCurve const* pC, Triangle2D const& T2, real_type offs_C, real_type& ss1,
    real_type& ss2 ) const
{
    ...
    for ( integer i{ 0 }; i < m_max_iter && !converged; ++i ) {
        ...
        */
        real_type const det = t2[0] * t1[1] - t1[0] * t2[1];
        real_type const px = p2[0] - p1[0];
        real_type const py = p2[1] - p1[1];
...
void ClothoidCurve::intersect_ISO(
    real_type offs, ClothoidCurve const& C, real_type offs_C, IntersectList& ilist ) const
{
    real_type ss1, ss2;
    G2LIB_DEBUG_TICTOC;
    ...
    if ( intersect_with_AABBtree ) {
        ...
        for ( const auto& [fst, snd] : intersectList ) {
            integer i = fst;
            UTILS_ASSERT_DEBUG( i >= 0 && i < static_cast<integer>( m_aabb_triangles.size() ),
                "ClothoidCurve::intersect_ISO( offs={}, C, offs_C={}, ilist ) i={} out of range "
                "[0,{})\n",
                offs, offs_C, i, m_aabb_triangles.size() );
            Triangle2D const& T1 = m_aabb_triangles.at( i );
            for ( integer j : snd ) {
                UTILS_ASSERT_DEBUG( j >= 0 && j < static_cast<integer>( C.m_aabb_triangles.size() ),
                    "ClothoidCurve::intersect_ISO( offs={}, C, offs_C={}, ilist ) j={} out of "
                    "range [0,{})\n",
                    offs, offs_C, j, C.m_aabb_triangles.size() );
                Triangle2D const& T2{ C.m_aabb_triangles.at( j ) };
        ...
        C.bb_triangles_ISO( offs_C, C.m_aabb_triangles, Utils::m_pi / 18, 1e100, 0 );
...
void ClothoidCurve::intersect( BaseCurve const* pC, IntersectList& ilist ) const
{
    if ( pC->type() == CurveType::CLOTHOID ) {
        ClothoidCurve const& C = *dynamic_cast<ClothoidCurve const*>( pC );
...
void ClothoidCurve::closest_point_internal( real_type s_begin, real_type s_end, real_type qx,
    real_type qy, real_type offs, real_type& x, real_type& y, real_type& s, real_type& dst ) const
{
#if 1
    ...
    for ( integer iter = 0; iter < m_max_iter; ++iter ) {
        ...
        real_type const kk = m_CD.kappa( s );
        real_type const sc = 1 + kk * offs;
...
void ClothoidCurve::closest_point_internal( real_type qx, real_type qy, real_type offs,
    real_type& x, real_type& y, real_type& s, real_type& DST ) const
{
    ...
    G2LIB_DEBUG_MESSAGE( "ClothoidCurve::closest_point_internal: build_AABBtree_ISO elapsed {}ms\n",
        tictoc.elapsed_ms() );
    ...
    if ( m_aabb_tree.num_tree_nodes() > G2LIB_AABB_MIN_NODES && intersect_with_AABBtree ) {
        ...
        G2LIB_DEBUG_MESSAGE( "ClothoidCurve::closest_point_internal min_distance_candidates "
                             "elapsed {}ms candidated #{}\n",
            tictoc.elapsed_ms(), candidateList.size() );

        UTILS_ASSERT0(
            !candidateList.empty(), "ClothoidCurve::closest_point_internal no candidate\n" );

        G2LIB_DEBUG_TIC;
        for ( integer ipos : candidateList ) {
            UTILS_ASSERT_DEBUG( ipos >= 0 && ipos < static_cast<integer>( m_aabb_triangles.size() ),
                "ClothoidCurve::closest_point_internal( qx={}, qy={}, offs={}, x, y, s, DST ) "
                "ipos={} out of range [0,{})\n",
...
integer ClothoidCurve::closest_point_ISO( real_type const qx, real_type const qy,
    ...
    real_type& DST ) const
{
    ...
    G2LIB_DEBUG_MESSAGE( "Clothoid::closest_point_ISO\n"
                         "||P-P0|| = {} and {}, |(P-P0).T| = {}\n",
...
real_type ClothoidCurve::theta_total_variation() const
{
    ...
    if ( kL * kR < 0 ) {
        ...
        if ( root > 0 && root < m_L ) {
            real_type const thM = m_CD.delta_theta( root );
...
real_type ClothoidCurve::theta_min_max( real_type& thMin, real_type& thMax ) const
{
    ...
    if ( kL * kR < 0 ) {
        real_type const root{ -m_CD.m_kappa0 / m_CD.m_dk };
        if ( root > 0 && root < m_L ) {
...
real_type ClothoidCurve::curvature_min_max( real_type& kMin, real_type& kMax ) const
{
    // cerco punto minimo parabola
    // root = -k/dk;
    kMin = m_CD.m_kappa0;
...
real_type ClothoidCurve::curvature_total_variation() const
{
    // cerco punto minimo parabola
    // root = -k/dk;
    real_type const km = m_CD.m_kappa0;
    real_type const kp = m_CD.kappa( m_L );
    return abs( kp - km );
}
...
{
    return m_L *
        ( m_CD.m_kappa0 * ( m_CD.m_kappa0 + m_L * m_CD.m_dk ) +
            ( m_L * m_L ) * m_CD.m_dk * m_CD.m_dk / 3 );
}

// - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -

real_type ClothoidCurve::integral_jerk2() const
{
    real_type const k2 = m_CD.m_kappa0 * m_CD.m_kappa0;
    real_type const k3 = m_CD.m_kappa0 * k2;
    real_type const k4 = k2 * k2;
    real_type const t1 = m_L;
    real_type const t2 = m_L * t1;
    real_type const t3 = m_L * t2;
    real_type const t4 = m_L * t3;
    return ( ( ( ( t4 / 5 * m_CD.m_dk + t3 * m_CD.m_kappa0 ) * m_CD.m_dk + ( 1 + 2 * t2 ) * k2 ) *
                     m_CD.m_dk +
                 2 * t1 * k3 ) *
                   m_CD.m_dk +
               k4 ) *
...
real_type ClothoidCurve::integral_snap2() const
{
    real_type const k2 = m_CD.m_kappa0 * m_CD.m_kappa0;
    real_type const k3 = m_CD.m_kappa0 * k2;
    real_type const k4 = k2 * k2;
    real_type const k5 = k4 * m_CD.m_kappa0;
    real_type const k6 = k4 * k2;
    real_type const dk2 = m_CD.m_dk * m_CD.m_dk;
    real_type const dk3 = m_CD.m_dk * dk2;
    real_type const dk4 = dk2 * dk2;
    real_type const dk5 = dk4 * m_CD.m_dk;
    real_type const dk6 = dk4 * dk2;
    real_type const t2 = m_L;
    real_type const t3 = m_L * t2;
    real_type const t4 = m_L * t3;
    real_type const t5 = m_L * t4;
    real_type const t6 = m_L * t5;
    real_type const t7 = m_L * t6;

    return ( ( t7 / 7 ) * dk6 + dk5 * m_CD.m_kappa0 * t6 + 3 * dk4 * k2 * t5 + 5 * dk3 * k3 * t4 +
               5 * dk2 * k4 * t3 + 3 * dk3 * t3 + 3 * m_CD.m_dk * k5 * t2 +
               9 * dk2 * m_CD.m_kappa0 * t2 + k6 + 9 * k2 * m_CD.m_dk ) *
        m_L;
}
...
ostream_type& operator<<( ostream_type& stream, ClothoidCurve const& c )
{
    fmt::print( stream,
        ...
        "κ₀ = {:<12} κ₁ = {:<12}\n"
        "∆κ = {:<12} L  = {:<12}\n",
        fmt::format( "{:.6}", c.x_begin() ), fmt::format( "{:.6}", c.x_end() ),
        fmt::format( "{:.6}", c.y_begin() ), fmt::format( "{:.6}", c.y_end() ),
        fmt::format( "{:.6}", c.theta_begin() ), fmt::format( "{:.6}", c.theta_end() ),
        fmt::format( "{:.6}", c.kappa_begin() ), fmt::format( "{:.6}", c.kappa_end() ),
        fmt::format( "{:.6}", c.m_CD.m_dk ), fmt::format( "{:.6}", c.m_L ) );
    return stream;
}
#endif
} // namespace G2lib
