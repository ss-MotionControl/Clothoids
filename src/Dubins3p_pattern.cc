...
void Dubins3p::get_sample_angles( real_type const xi, real_type const yi, real_type const thetai,
    real_type const xm, real_type const ym, real_type const xf, real_type const yf,
    real_type const thetaf, real_type const k_max, real_type const tolerance,
    vector<real_type>& angles ) const
{
    ...
    angles.reserve( 2 * NSEG + 12 );
    {
        real_type ang[12];
        integer npts{
            this->get_range_angles( xi, yi, thetai, xm, ym, xf, yf, thetaf, k_max, ang ) };
        if ( npts > 0 ) {
...
bool Dubins3p::build_pattern_search( real_type xi, real_type yi, real_type thetai, real_type xm,
    real_type ym, real_type xf, real_type yf, real_type thetaf, real_type k_max,
    real_type const tolerance, bool const use_trichotomy, bool const use_bracket )
{
    m_evaluation = 0;
    ...
    typedef struct Dubins3p_data {
        ...
        void copy( Dubins3p_data const& rhs )
        {
            D0.copy( rhs.D0 );
            D1.copy( rhs.D1 );
            thetam = rhs.thetam;
    ...
    auto check_change_sign = []( Dubins3p_data const& A, Dubins3p_data const& B ) -> bool {
        bool ok{ A.D0.icode() == B.D0.icode() && A.D1.icode() == B.D1.icode() };
        if ( ok ) ok = A.len_D * B.len_D <= 0;
        return ok;
    };

    auto eval3p = [this, xi, yi, thetai, xm, ym, xf, yf, thetaf, k_max, tolerance, use_trichotomy,
                      use_bracket]( Dubins3p_data& D3P, real_type const thetam ) -> void {
        D3P.thetam = thetam;
    ...
    auto eval_for_bracket = [this, xi, yi, thetai, xm, ym, xf, yf, thetaf, k_max, tolerance,
                                use_trichotomy, use_bracket]( real_type const theta ) -> real_type {
        Dubins D0{ "temporary Dubins A" };
        Dubins D1{ "temporary Dubins B" };
    ...
    auto do_bracket = [&check_change_sign, &eval3p, &eval_for_bracket](
                          Dubins3p_data& L, Dubins3p_data& C, Dubins3p_data& R ) -> bool {
    ...
    auto bracketing = [&eval3p, &do_bracket, &use_bracket](
                          Dubins3p_data& A, Dubins3p_data& P3, Dubins3p_data& B ) -> void {
        bool ok{ use_bracket };
    ...
    auto simple_search = [&eval3p, &do_bracket, use_bracket](
                             Dubins3p_data& L, Dubins3p_data& C, Dubins3p_data& R ) -> void {
        bool ok{ use_bracket };
