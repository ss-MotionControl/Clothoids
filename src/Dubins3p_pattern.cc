...
namespace G2lib
{
  ...
  bool Dubins3p::build_pattern_search(
    real_type       xi,
    real_type       yi,
    real_type       thetai,
    real_type       xm,
    ...
    bool const      use_bracket )
  {
    ...
    auto eval3p = [this, xi, yi, thetai, xm, ym, xf, yf, thetaf, k_max, tolerance, use_trichotomy, use_bracket](
                    Dubins3p_data & D3P,
                    real_type const thetam ) -> void
    {
      D3P.thetam = thetam;
    ...
    auto eval_for_bracket =
      [this, xi, yi, thetai, xm, ym, xf, yf, thetaf, k_max, tolerance, use_trichotomy, use_bracket](
        real_type const theta ) -> real_type
    {
      Dubins D0{ "temporary Dubins A" };
      Dubins D1{ "temporary Dubins B" };
    ...
    auto do_bracket = [&check_change_sign,
                       &eval3p,
                       &eval_for_bracket]( Dubins3p_data & L, Dubins3p_data & C, Dubins3p_data & R ) -> bool
    {
    ...
    auto bracketing =
      [&eval3p, &do_bracket, &use_bracket]( Dubins3p_data & A, Dubins3p_data & P3, Dubins3p_data & B ) -> void
    {
      bool ok{ use_bracket };
    ...
    auto simple_search =
      [&eval3p, &do_bracket, use_bracket]( Dubins3p_data & L, Dubins3p_data & C, Dubins3p_data & R ) -> void
    {
      bool ok{ use_bracket };
    ...
    for ( real_type const a : angles )
    {
      ...
      if ( C.len <= L.len && C.len <= R.len )
      {
        if ( C.len < 1.5 * lmin )
        {
          ...
          if ( use_trichotomy )
          {
            while ( RR.thetam - LL.thetam > tolerance && m_evaluation < m_max_evaluation ) bracketing( LL, CC, RR );
          }
          else
          {
            while ( RR.thetam - LL.thetam > tolerance && m_evaluation < m_max_evaluation ) simple_search( LL, CC, RR );
          }
          if ( CC.len < lmin )
          {
            lmin = CC.len;
