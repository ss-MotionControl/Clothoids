...
// date: August 11, 2005
*/
static constexpr real_type fn[] = { 0.49999988085884732562, 1.3511177791210715095,
    1.3175407836168659241, 1.1861149300293854992, 0.7709627298888346769, 0.4173874338787963957,
    0.19044202705272903923, 0.06655998896627697537, 0.022789258616785717418,
    0.0040116689358507943804, 0.0012192036851249883877 };

static constexpr real_type fd[] = { 1.0, 2.7022305772400260215, 4.2059268151438492767,
    4.5221882840107715516, 3.7240352281630359588, 2.4589286254678152943, 1.3125491629443702962,
    0.5997685720120932908, 0.20907680750378849485, 0.07159621634657901433, 0.012602969513793714191,
    0.0038302423512931250065 };

static constexpr real_type gn[] = { 0.50000014392706344801, 0.032346434925349128728,
    0.17619325157863254363, 0.038606273170706486252, 0.023693692309257725361,
    0.007092018516845033662, 0.0012492123212412087428, 0.00044023040894778468486,
    -8.80266827476172521e-6, -1.4033554916580018648e-8, 2.3509221782155474353e-10 };

static constexpr real_type gd[] = { 1.0, 2.0646987497019598937, 2.9109311766948031235,
    2.6561936751333032911, 2.0195563983177268073, 1.1167891129189363902, 0.57267874755973172715,
...
// -------------------------------------------------------------------------
// nk max 3
static void evalXYaLarge(
    integer const nk, real_type const a, real_type const b, real_type X[], real_type Y[] )
{
    UTILS_ASSERT(
        nk < 4 && nk > 0, "In evalXYaLarge first argument nk must be in 1..3, nk {}\n", nk );
...
static void evalXYaSmall(
    real_type const a, real_type const b, integer const p, real_type& X, real_type& Y )
{
...
static void evalXYaSmall( integer const nk, real_type const a, real_type const b, integer const p,
...
void GeneralizedFresnelCS(
    real_type const a, real_type const b, real_type const c, real_type& intC, real_type& intS )
{
    real_type xx, yy;
    if ( abs( a ) < A_THRESOLD )
...
void GeneralizedFresnelCS( integer const nk, real_type const a, real_type const b,
    real_type const c, real_type intC[], real_type intS[] )
{
...
void ClothoidData::evaluate(
    real_type const s, real_type& theta, real_type& kappa, real_type& x, real_type& y ) const
{
    real_type C, S;
    real_type const sdk = s * m_dk;
...
void ClothoidData::eval_ISO_DDD(
...
bool ClothoidData::bbTriangle( real_type const L, real_type& xx0, real_type& yy0, real_type& xx1,
...
bool ClothoidData::bbTriangle_ISO( real_type const L, real_type const offs, real_type& xx0,
    real_type& yy0, real_type& xx1, real_type& yy1, real_type& xx2, real_type& yy2 ) const
{
    real_type const theta_max = theta( L );
    real_type const theta_min = m_theta0;
    real_type const dtheta = abs( theta_max - theta_min );
...
int ClothoidData::build_G1( real_type const _x0, real_type const _y0, real_type const _theta0,
    real_type const x1, real_type const y1, real_type const theta1, real_type const tol,
    real_type& L, bool const compute_deriv, real_type L_D[2], real_type k_D[2], real_type dk_D[2] )
{
    ...
    Y *= Y;
    X *= X;
    real_type A{ ( phi0 + phi1 ) *
        ( CF[0] + xy * ( CF[1] + xy * CF[2] ) + ( CF[3] + xy * CF[4] ) * ( X + Y ) +
            CF[5] * ( X * X + Y * Y ) ) };
    // newton
...
#ifndef DOXYGEN_SHOULD_SKIP_THIS
bool ClothoidData::build_forward( real_type const x0, real_type const y0, real_type const theta0,
