...
namespace G2lib
{
  ...
  void LineSegment::bb_triangles( vector<Triangle2D> & tvec, real_type max_angle, real_type max_size, integer icurve )
    const
  {
  ...
  void LineSegment::bb_triangles_ISO(
    real_type            offs,
  ...
  static integer orientation( real_type const p[2], real_type const q[2], real_type const r[2], real_type const epsi )
  {
  ...
  bool LineSegment::collision_ISO( real_type const offs, LineSegment const & S, real_type const S_offs ) const
  {
    // The main function that returns true if line segment 'p1q1'
    // and 'p2q2' intersect.
    L_struct L1;
  ...
  bool LineSegment::collision_ISO( real_type const offs, BaseCurve const * pC, real_type const offs_C ) const
  {
    if ( pC->type() == CurveType::LINE )
    {
      LineSegment const & LS = *static_cast<LineSegment const*>(pC);
  ...
  integer LineSegment::closest_point_ISO(
    real_type const qx,
    real_type const qy,
    real_type     & x,
    real_type     & y,
