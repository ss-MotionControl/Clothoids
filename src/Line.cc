...
void LineSegment::bb_triangles(
    vector<Triangle2D>& tvec, real_type max_angle, real_type max_size, integer icurve ) const
{
    ...
    if ( xmax > xmin || ymax > ymin ) {
        ...
        UTILS_ERROR( "LineSegment bb_triangles found a degenerate line\n"
...
void LineSegment::bb_triangles_ISO( real_type offs, vector<Triangle2D>& tvec, real_type max_angle,
    real_type max_size, integer icurve ) const
{
    real_type xmin, ymin, xmax, ymax;
...
static bool onSegment(
    real_type const p[2], real_type const q[2], real_type const r[2], real_type const epsi )
{
    real_type mi_x, ma_x;
    if ( p[0] > r[0] ) {
        mi_x = r[0];
...
static bool intersect(
    real_type const epsi, L_struct const& L1, L_struct const& L2, real_type& s1, real_type& s2 )
{
...
static bool collision( real_type const epsi, L_struct const& L1, L_struct const& L2 )
{
    // Find the four orientations needed for general and special cases
    integer const o1{ orientation( L1.p, L1.q, L2.p, epsi ) };
    integer const o2{ orientation( L1.p, L1.q, L2.q, epsi ) };
...
void LineSegment::rotate( real_type const angle, real_type const cx, real_type const cy )
{
...
void LineSegment::paramNURBS( integer& n_knots, integer& n_pnts ) const
{
    n_pnts = 2;
    n_knots = 4;
}
...
void LineSegment::toBS( real_type knots[], real_type Poly[][2] ) const
{
    knots[0] = knots[1] = 0;
    knots[2] = knots[3] = 1;
    Poly[0][0] = m_x0;
...
bool LineSegment::intersect_ISO( real_type const offs, LineSegment const& S, real_type const S_offs,
    real_type& s1, real_type& s2 ) const
{
    // The main function that returns true if line segment 'p1q1'
    // and 'p2q2' intersect.
    L_struct L1;
...
bool LineSegment::collision_ISO(
    real_type const offs, LineSegment const& S, real_type const S_offs ) const
{
    // The main function that returns true if line segment 'p1q1'
    // and 'p2q2' intersect.
    L_struct L1;
...
bool LineSegment::collision( BaseCurve const* pC ) const
{
...
void LineSegment::intersect( LineSegment const& LS, IntersectList& ilist ) const
{
    real_type s1, s2;
...
void LineSegment::intersect_ISO( real_type const offs, LineSegment const& LS,
    real_type const offs_LS, IntersectList& ilist ) const
{
    real_type s1, s2;
...
void LineSegment::intersect( BaseCurve const* pC, IntersectList& ilist ) const
{
    if ( pC->type() == CurveType::LINE ) {
        LineSegment const& LS = *dynamic_cast<LineSegment const*>( pC );
...
void LineSegment::intersect_ISO(
...
integer LineSegment::closest_point_ISO( real_type const qx, real_type const qy, real_type& x,
...
integer LineSegment::closest_point_ISO( real_type const qx, real_type const qy,
