...
#include "BaseCurve_using.hxx"
    ...
    //!
    explicit BiarcList( string_view name ) : BaseCurve( name )
    {
    ...
    {
        return integer( m_biarc_list.size() );
    }
    ...
    //! \param[out] max_size  maximum admissible size of the covering tirnagles
    //!
    void build_AABBtree_SAE( real_type offs,
        real_type max_angle = Utils::m_pi / 6, // 30 degree
        real_type max_size = 1e100 ) const
    {
    ...
    //!
    void intersect_ISO(
        real_type offs, BiarcList const& BL, real_type offs_obj, IntersectList& ilist ) const;
