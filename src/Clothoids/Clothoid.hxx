...
class ClothoidCurve : public BaseCurve {
  ...
  private:
    ...
    void optimized_sample_internal_ISO( real_type s_begin, real_type s_end, real_type offs,
        real_type ds, real_type max_angle, vector<real_type>& s ) const;

    void bb_triangles_internal_ISO( real_type offs, vector<Triangle2D>& tvec, real_type s_begin,
        real_type s_end, real_type max_angle, real_type max_size, integer icurve ) const;

    void closest_point_internal( real_type s_begin, real_type s_end, real_type qx, real_type qy,
        real_type offs, real_type& x, real_type& y, real_type& s, real_type& dst ) const;

    void closest_point_internal( real_type qx, real_type qy, real_type offs, real_type& x,
    ...
    bool aabb_intersect_ISO( Triangle2D const& T1, real_type offs, ClothoidCurve const* pC,
        Triangle2D const& T2, real_type C_offs, real_type& ss1, real_type& ss2 ) const;
...
#include "BaseCurve_using.hxx"
    ...
    //!
    explicit ClothoidCurve( real_type x0, real_type y0, real_type theta0, real_type k, real_type dk,
        real_type L, string_view name );
    ...
    //! \param[in] name   name of the clothoid curve
    //!
    explicit ClothoidCurve( real_type const P0[], real_type theta0, real_type const P1[],
        real_type theta1, string const& name );
    ...
    //!
    int build_G1( real_type x0, real_type y0, real_type theta0, real_type x1, real_type y1,
        real_type theta1, real_type tol = 1e-12 );
    ...
    //! \return number of iteration performed
    //!
    int build_G1_D( real_type x0, real_type y0, real_type theta0, real_type x1, real_type y1,
        real_type theta1, real_type L_D[2], real_type k_D[2], real_type dk_D[2],
        real_type tol = 1e-12 );
    ...
    bool build_forward( real_type x0, real_type y0, real_type theta0, real_type kappa0,
        real_type x1, real_type y1, real_type tol = 1e-12 );
    ...
    void build( ClothoidCurve const& C )
    {
        this->copy( C );
    }
    ...
    void optimized_sample_ISO(
        real_type offs, integer npts, real_type max_angle, vector<real_type>& s ) const;
    ...
    //! (if angle variation less that \f$ \pi/2 \f$)
    //!
    bool bbTriangle_ISO( real_type offs, real_type& xx0, real_type& yy0, real_type& xx1,
    ...
    //! (if angle variation less that \f$ \pi/2 \f$)
    //!
    bool bbTriangle_SAE( real_type offs, real_type& xx0, real_type& yy0, real_type& xx1,
        real_type& yy1, real_type& xx2, real_type& yy2 ) const
    {
    ...
    {
        return m_CD.m_theta0;
    }
    real_type kappa_begin() const override
    {
        return m_CD.m_kappa0;
    }
    ...
    // collision detection
    bool approximate_collision_ISO( real_type offs, ClothoidCurve const& C, real_type offs_C,
        real_type max_angle, real_type max_size ) const;
