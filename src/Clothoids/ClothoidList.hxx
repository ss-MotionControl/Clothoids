...
class G2solve2arc {
    ...
    void evalG( real_type alpha, real_type L, real_type th, real_type k, real_type G[2],
        real_type G_1[2], real_type G_2[2] ) const;
  ...
  public:
    ...
    //!
    int build( real_type x0, real_type y0, real_type theta0, real_type kappa0, real_type x1,
        real_type y1, real_type theta1, real_type kappa1 );
...
class G2solve3arc {
    ...
    // precomputed values
    real_type m_K0{ real_type( 0 ) }, m_K1{ real_type( 0 ) }, m_c0{ real_type( 0 ) },
        m_c1{ real_type( 0 ) }, m_c2{ real_type( 0 ) }, m_c3{ real_type( 0 ) },
        m_c4{ real_type( 0 ) }, m_c5{ real_type( 0 ) }, m_c6{ real_type( 0 ) },
        m_c7{ real_type( 0 ) }, m_c8{ real_type( 0 ) }, m_c9{ real_type( 0 ) },
        m_c10{ real_type( 0 ) }, m_c11{ real_type( 0 ) }, m_c12{ real_type( 0 ) },
        m_c13{ real_type( 0 ) }, m_c14{ real_type( 0 ) };
  ...
  public:
    ...
    //! \param[in] dmax    rough desidered maximum angle divergence from guess, if 0 computed
    //! automatically \return number of iteration, -1 if fails
    //!
    //!
    int build( real_type x0, real_type y0, real_type theta0, real_type kappa0, real_type x1,
        real_type y1, real_type theta1, real_type kappa1, real_type Dmax = 0, real_type dmax = 0 );
    ...
    //!
    //!
    int build_fixed_length( real_type s0, real_type x0, real_type y0, real_type theta0,
        real_type kappa0, real_type s1, real_type x1, real_type y1, real_type theta1,
        real_type kappa1 );
    ...
    {
        return m_S0.length() + m_S1.length() + m_SM.length();
    }
    ...
    {
        return m_S0.theta_total_variation() + m_S1.theta_total_variation() +
    ...
    {
        return m_S0.curvature_total_variation() + m_S1.curvature_total_variation() +
    ...
    {
        return m_S0.integral_jerk2() + m_S1.integral_jerk2() + m_SM.integral_jerk2();
    }
    ...
    {
        return m_S0.integral_snap2() + m_S1.integral_snap2() + m_SM.integral_snap2();
    }
    ...
    //!
    //! Return angle second derivative (curvature derivative) as a function of curvilinear
    //! coordinate
    //!
...
class ClothoidList : public BaseCurve {
    ...
    integer closest_point_internal( real_type qx, real_type qy, real_type offs, real_type& x,
...
#include "BaseCurve_using.hxx"
    ...
    //! Build an empty clothoid list
    //!
    explicit ClothoidList( string_view name ) : BaseCurve( name )
    {
    ...
    {
        return CurveType::CLOTHOID_LIST;
    }
    ...
    //! \param L      length of the segment
    //!
    void push_back( real_type x0, real_type y0, real_type theta0, real_type kappa0,
        real_type dkappa, real_type L );
    ...
    //! \param theta1 final angle
    //!
    void push_back_G1( real_type x0, real_type y0, real_type theta0, real_type x1, real_type y1,
        real_type theta1 );
    ...
    {
        return this->y_end() - this->y_begin();
    }
    ...
    {
        return this->tx_end() - this->tx_begin();
    }
    ...
    {
        return this->ty_end() - this->ty_begin();
    }
    ...
    //!
    bool smooth_quasi_G2( integer max_iter, real_type epsi, real_type& max_dK );
    ...
    //! \return true if curve is closed
    //!
    bool build( real_type x0, real_type y0, real_type theta0, integer n, real_type const s[],
    ...
    //! \return true if curve is closed
    //!
    bool build( real_type x0, real_type y0, real_type theta0, vector<real_type> const& s,
    ...
    //!
    bool build_raw( integer n, real_type const x[], real_type const y[], real_type const abscissa[],
    ...
    //! \return false if fails
    //!
    bool build_raw( vector<real_type> const& x, vector<real_type> const& y,
        vector<real_type> const& abscissa, vector<real_type> const& theta,
        vector<real_type> const& kappa )
    {
        integer n = integer( x.size() );
        if ( n != integer( y.size() ) || n != integer( abscissa.size() ) ||
            n != integer( theta.size() ) || n != integer( kappa.size() ) )
            return false;
        return build_raw(
            n, &x.front(), &y.front(), &abscissa.front(), &theta.front(), &kappa.front() );
    }
    ...
    {
        return integer( m_clothoid_list.size() );
    }
    ...
    void bb_triangles( vector<Triangle2D>& tvec,
        ...
        real_type max_size = 1e100, integer icurve = 0 ) const override;
    ...
    void evaluate(
        real_type s, real_type& th, real_type& k, real_type& x, real_type& y ) const override;

    // . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . .

    void evaluate_ISO( real_type s, real_type offs, real_type& th, real_type& k, real_type& x,
    ...
    //! \return n >= 0 point is projected orthogonal, n is the number of the segment at minimum
    //! distance<br>
    ...
    //! \param  t     curvilinear coordinate of the point x,y (if orthogonal projection)
    //! \param  dst   distance point projected point
    //! \return n > 0 point is projected orthogonal, n-1 is the number of the segment at minimum
    //! distance<br>
    //!        -(n+1) minimum point is not othogonal projection to curve
    //!
    ...
    //!
    integer closest_point_in_range_ISO( real_type qx, real_type qy, integer icurve_begin,
        integer icurve_end, real_type& x, real_type& y, real_type& s, real_type& t, real_type& dst,
    ...
    //!
    integer closest_point_in_range_SAE( real_type qx, real_type qy, integer icurve_begin,
        integer icurve_end, real_type& x, real_type& y, real_type& s, real_type& t, real_type& dst,
        integer& icurve ) const
    {
    ...
    //!
    integer closest_point_in_s_range_ISO( real_type qx, real_type qy, real_type s_begin,
        real_type s_end, real_type& x, real_type& y, real_type& s, real_type& t, real_type& dst,
        integer& icurve ) const;
    ...
    //! \return 1 ok -1 projection failed
    //!
    integer closest_point_in_s_range_SAE( real_type qx, real_type qy, real_type s_begin,
        real_type s_end, real_type& x, real_type& y, real_type& s, real_type& t, real_type& dst,
        integer& icurve ) const
    {
        integer res =
            this->closest_point_in_s_range_ISO( qx, qy, s_begin, s_end, x, y, s, t, dst, icurve );
        t = -t;
        return res;
    }
    ...
    //!
    integer findST1(
        integer ibegin, integer iend, real_type x, real_type y, real_type& s, real_type& t ) const;
    ...
    //!
    void intersect_ISO(
        real_type offs, ClothoidList const& CL, real_type offs_obj, IntersectList& ilist ) const;
...
class ClothoidSplineG2 {
  ...
  private:
    ...
    real_type diff2pi( real_type in ) const
    {
