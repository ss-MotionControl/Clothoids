...
namespace G2lib
{
  ...
  static auto Newton =
    []( integer n, integer max_iter, real_type dump_min, real_type tolerance, real_type theta[], auto & F, auto & F_JF )
    -> bool
  {
  ...
  bool ClothoidSplineG2::build_P1(
    integer const   n,
    ...
    real_type       theta_end )
  {
    ...
    {
      Vector theta_min( m_npts ), theta_max( m_npts ), omega( m_npts ), len( m_npts );
    ...
    auto F_JF =
      [this, &ne, &ne1, &theta_init, &theta_end]( Vector const & theta, Vector & F, SparseMatrix & JF ) -> bool
    {
      JF.setZero();
  ...
  bool ClothoidSplineG2::build_P2( integer const n, real_type const xvec[], real_type const yvec[], real_type theta[] )
  {
    allocate( n );
