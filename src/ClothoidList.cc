...
namespace G2lib
{
  ...
  ClothoidList::ClothoidList( BaseCurve const * pC ) : BaseCurve( pC->name() )
  {
    ...
    case CurveType::BIARC_LIST:
      ...
      break;
      case CurveType::CLOTHOID_LIST: this->copy( *static_cast<ClothoidList const *>( pC ) ); break;
      case CurveType::DUBINS: this->push_back( *static_cast<Dubins const *>( pC ) ); break;
  ...
  void ClothoidList::copy( ClothoidList const & L )
  {
    ...
    std::copy( L.m_clothoid_list.begin(), L.m_clothoid_list.end(), back_inserter( m_clothoid_list ) );
  ...
  void ClothoidList::adjust_angles()
  {
    if ( !m_clothoid_list.empty() )
    {
  ...
  bool ClothoidList::build_G1( integer const n, real_type const x[], real_type const y[], real_type const theta[] )
  {
    UTILS_ASSERT0( n > 1, "ClothoidList::build_G1, at least 2 points are necessary\n" );
    ...
    real_type const kappa_end )
  {
    ...
    iter = G2_3arc.build( x[0], y[0], theta_init, kappa_init, x[1], y[1], c.theta_end(), c.kappa_end() );
    ...
    iter =
      G2_3arc.build( ce.x_end(), ce.y_end(), ce.theta_end(), ce.kappa_end(), x[nm1], y[nm1], theta_end, kappa_end );
  ...
  bool ClothoidList::build_G2_cyclic( integer const n, real_type const x[], real_type const y[] )
  {
    using Vector = Eigen::Matrix<real_type, Eigen::Dynamic, 1>;
  ...
  bool ClothoidList::smooth_quasi_G2( integer const max_iter, real_type const epsi, real_type & max_dK )
  {
    ...
    auto do_smooth = []( ClothoidCurve * CL, ClothoidCurve * CR ) -> real_type
    {
      ...
      real_type const               theta{
        solver.eval2( theta1 - Utils::m_pi / 20, theta1 + Utils::m_pi / 20, -Utils::m_pi / 2, Utils::m_pi / 2, fun )
      };
      fun(theta);
      return std::abs(dK);
    };
  ...
  void ClothoidList::bb_triangles(
    vector<Triangle2D> & tvec,
    real_type const      max_angle,
    real_type const      max_size,
    integer const        icurve ) const
  {
  ...
  void ClothoidList::bb_triangles_ISO(
  ...
  void ClothoidList::eval_ISO_D( real_type s, real_type const offs, real_type & x_D, real_type & y_D ) const
  {
    integer const idx{ find_at_s( s ) };
    ClothoidCurve const & c{ get( idx ) };
  ...
  void ClothoidList::eval_ISO_DD( real_type s, real_type const offs, real_type & x_DD, real_type & y_DD ) const
  {
    integer const idx{ find_at_s( s ) };
    ClothoidCurve const & c{ get( idx ) };
  ...
  void ClothoidList::eval_ISO_DDD( real_type s, real_type const offs, real_type & x_DDD, real_type & y_DDD ) const
  {
    integer const idx{ find_at_s( s ) };
    ClothoidCurve const & c{ get( idx ) };
  ...
  void ClothoidList::trim( real_type s_begin, real_type s_end, ClothoidList & newCL ) const
  {
    ...
    if ( s_begin < s_end )
    {
      ...
      else
      {
        ...
        for ( ++i_begin; i_begin < i_end; ++i_begin ) newCL.push_back( m_clothoid_list[i_begin] );
    ...
    const
  {
    ...
    if (
      m_aabb_done && Utils::is_zero( offs - m_aabb_offs ) && Utils::is_zero( max_angle - m_aabb_max_angle ) &&
  ...
  bool ClothoidList::collision_ISO( real_type const offs, BaseCurve const * pC, real_type const offs_C ) const
  {
    if ( pC->type() == CurveType::CLOTHOID_LIST )
    {
      ClothoidList const & C{ *static_cast<ClothoidList const *>(pC) };
  ...
  bool ClothoidList::collision_ISO( real_type const offs, ClothoidList const & CL, real_type const offs_CL ) const
  {
    G2LIB_DEBUG_TICTOC;
    ...
    G2LIB_DEBUG_TOC;
    G2LIB_DEBUG_MESSAGE( "ClothoidList collision_ISO: collisions elapsed {}ms\n", tictoc.elapsed_ms() );
    ...
    if ( intersect_with_AABBtree )
    {
      ...
      for ( const auto & [fst, snd] : intersectList )
      {
        ...
        for ( integer j : snd )
        {
          ...
          real_type ss1, ss2;
          bool const converged{ C1.aabb_intersect_ISO( T1, offs, &C2, T2, offs_CL, ss1, ss2 ) };
      ...
      G2LIB_DEBUG_TOC;
      G2LIB_DEBUG_MESSAGE( "ClothoidList::intersect_ISO: intersect elapsed {}ms\n", tictoc.elapsed_ms() );
    }
    else
    {
      G2LIB_DEBUG_TIC;
      ...
      G2LIB_DEBUG_TOC;
      G2LIB_DEBUG_MESSAGE( "ClothoidCurve::intersect_ISO: collisions elapsed {}ms noAABB\n", tictoc.elapsed_ms() );
    }
  }
  ...
  integer ClothoidList::closest_point_internal(
    real_type const qx,
    real_type const qy,
    real_type const offs,
    real_type     & x,
    ...
    real_type &     DST ) const
  {
    ...
    if ( m_aabb_tree.num_tree_nodes() > G2LIB_AABB_MIN_NODES && intersect_with_AABBtree )
    {
      ...
      UTILS_ASSERT0( !candidateList.empty(), "ClothoidList::closest_point_internal no candidate\n" );
      ...
      G2LIB_DEBUG_TOC;
      G2LIB_DEBUG_MESSAGE( "ClothoidList::closest_point_internal minimize elapsed {}ms\n", tictoc.elapsed_ms() );
    }
    else
    {
      G2LIB_DEBUG_TIC;
  ...
  integer ClothoidList::closest_point_ISO(
    real_type const qx,
    real_type const qy,
    real_type const offs,
    real_type &     x,
  ...
  integer ClothoidList::closest_segment( real_type const qx, real_type const qy ) const
  {
    ...
    G2LIB_DEBUG_MESSAGE( "ClothoidList::closest_segment build_AABBtree_ISO elapsed {}ms\n", tictoc.elapsed_ms() );
    ...
    else
    {
      ...
      G2LIB_DEBUG_TOC;
      G2LIB_DEBUG_MESSAGE( "ClothoidList::closest_segment elapsed {}ms noAABB\n", tictoc.elapsed_ms() );
    }
    return icurve;
  }
  ...
  integer ClothoidList::closest_point_in_range_ISO(
    real_type const qx,
    real_type const qy,
  ...
  {
    UTILS_ASSERT0( !m_clothoid_list.empty(), "ClothoidList::closest_point_in_range_ISO, empty list\n" );
    integer const nsegs{ this->num_segments() };
    if ( nsegs == 1 )
    {  // only 1 segment to check
      icurve = 0;
    ...
    if ( ib < 0 ) ib += nsegs;
    if ( ie < 0 ) ie += nsegs;
    UTILS_ASSERT( ib >= 0 && ie >= 0, "ClothoidList::closest_point_in_range_ISO, ib = {} ie = {}\n", ib, ie );
    ...
    do
    {
      ...
      integer const C_res{ m_clothoid_list.at( iseg ).closest_point_ISO( qx, qy, C_x, C_y, C_s, C_t, C_dst ) };
      G2LIB_DEBUG_MESSAGE(
      ...
      if ( C_dst < dst )
      {
        ...
        icurve = iseg;
        res    = C_res;
        G2LIB_DEBUG_MESSAGE( "ClothoidList::closest_point_in_range_ISO, new min at s = {}, res = {}\n", s, res );
      }
    } while ( iseg != ie );
    return res;
  }
  ...
  integer ClothoidList::closest_point_in_s_range_ISO(
    real_type const qx,
    real_type const qy,
    real_type       s_begin,
    real_type       s_end,
    ...
    integer &       icurve ) const
  {
    UTILS_ASSERT0( !m_clothoid_list.empty(), "ClothoidList::closest_point_in_s_range_ISO, empty list\n" );
    // put in range
    ...
    else
    {
      ...
      if ( i_begin < i_end )
      {
        ...
        if ( dst1 < dst )
        {
          ...
          res    = res1;
          icurve = icurve1;
          G2LIB_DEBUG_MESSAGE( "ClothoidList::closest_point_in_s_range_ISO: new min at s = {}, res = {}\n", s, res );
        }
      }
    }
    return res;
  ...
  real_type ClothoidList::closest_point_by_sample(
    real_type const ds,
    real_type const qx,
    real_type const qy,
    real_type &     X,
  ...
  void ClothoidList::get_delta_kappa( real_type deltaKappa[] ) const
  {
    auto ic{ m_clothoid_list.begin() };
    integer k{0};
    for ( ++ic; ic != m_clothoid_list.end(); ++ic, ++k ) deltaKappa[k] = ic->kappa_begin() - ic[-1].kappa_end();
  }

  // - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -

  integer ClothoidList::findST1( real_type const x, real_type const y, real_type & s, real_type & t ) const
  {
