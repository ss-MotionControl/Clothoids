...
void ClothoidList::build( LineSegment const& LS )
{
...
void ClothoidList::build( CircleArc const& C )
{
    this->reset_last_interval();
    this->init();
    this->push_back( C );
...
void ClothoidList::build( Biarc const& C )
{
    this->reset_last_interval();
    this->init();
    this->push_back( C.C0() );
...
void ClothoidList::build( BiarcList const& c )
{
    this->reset_last_interval();
    this->init();
    this->push_back( c );
...
void ClothoidList::build( ClothoidCurve const& c )
{
    this->reset_last_interval();
    this->init();
    this->push_back( c );
...
void ClothoidList::build( PolyLine const& pl )
{
    this->reset_last_interval();
    this->init();
    this->push_back( pl );
...
void ClothoidList::build( ClothoidList const& pl )
{
    this->reset_last_interval();
    this->init();
    this->push_back( pl );
...
void ClothoidList::build( G2solve2arc const& C )
{
    this->reset_last_interval();
    this->init();
    this->push_back( C );
...
void ClothoidList::build( G2solve3arc const& C )
{
    this->reset_last_interval();
    this->init();
    this->push_back( C );
...
void ClothoidList::build( G2solveCLC const& C )
{
    this->reset_last_interval();
    this->init();
    this->push_back( C );
...
void ClothoidList::build( Dubins const& C )
{
    this->reset_last_interval();
    this->init();
    this->push_back( C );
...
integer ClothoidList::find_at_s( real_type& s ) const
{
...
#endif
    Utils::search_interval<integer, real_type>( static_cast<integer>( m_s0.size() ), m_s0.data(), s,
        last_interval, m_curve_is_closed, true );
...
void ClothoidList::reserve( integer const n )
{
    m_s0.reserve( n + 1 );
...
void ClothoidList::push_back( G2solve2arc const& c )
{
...
void ClothoidList::push_back( G2solve3arc const& c )
{
...
void ClothoidList::push_back( G2solveCLC const& c )
{
    m_s0.reserve( m_s0.size() + 3 );
    m_clothoid_list.reserve( m_clothoid_list.size() + 3 );
...
void ClothoidList::push_back( Dubins const& c )
{
    if ( m_clothoid_list.empty() ) m_s0.emplace_back( 0 );
    CircleArc const& C0 = c.C0();
    CircleArc const& C1 = c.C1();
...
void ClothoidList::push_back( Dubins3p const& c )
{
    if ( m_clothoid_list.empty() ) m_s0.emplace_back( 0 );
    CircleArc const& C0 = c.C0();
    CircleArc const& C1 = c.C1();
...
void ClothoidList::push_back( real_type const kappa0, real_type const dkappa, real_type const L )
{
...
void ClothoidList::push_back( real_type const x0, real_type const y0, real_type const theta0,
    real_type const kappa0, real_type const dkappa, real_type const L )
{
    ClothoidCurve c{ "ClothoidList::push_back temporary c" };
    c.build( x0, y0, theta0, kappa0, dkappa, L );
    push_back( c );
...
void ClothoidList::push_back_G1( real_type const x1, real_type const y1, real_type const theta1 )
{
    UTILS_ASSERT0( !m_clothoid_list.empty(), "ClothoidList::push_back_G1(...) empty list!\n" );
...
void ClothoidList::push_back_G1( real_type const x0, real_type const y0, real_type const theta0,
...
bool ClothoidList::build_G1( integer const n, real_type const x[], real_type const y[] )
{
...
bool ClothoidList::build_G1(
    integer const n, real_type const x[], real_type const y[], real_type const theta[] )
{
    UTILS_ASSERT0( n > 1, "ClothoidList::build_G1, at least 2 points are necessary\n" );
...
bool ClothoidList::smooth_quasi_G2(
    integer const max_iter, real_type const epsi, real_type& max_dK )
{
    ...
    auto do_smooth = []( ClothoidCurve* CL, ClothoidCurve* CR ) -> real_type {
        real_type const theta0{ CL->theta_begin() };
        real_type const x0{ CL->x_begin() };
        real_type const y0{ CL->y_begin() };
        real_type const theta1{ CL->theta_end() };
        real_type const x1{ CL->x_end() };
        real_type const y1{ CL->y_end() };
        real_type const theta2{ CR->theta_end() };
        real_type const x2{ CR->x_end() };
        real_type const y2{ CR->y_end() };
        real_type const dK{ CL->kappa_end() - CR->kappa_begin() };
        auto fun = [CL, CR, x0, y0, theta0, x1, y1, x2, y2, theta2]( real_type const theta ) {
        ...
        real_type const theta{ solver.eval2( theta1 - Utils::m_pi / 20, theta1 + Utils::m_pi / 20,
            -Utils::m_pi / 2, Utils::m_pi / 2, fun ) };
        fun( theta );
        return std::abs( dK );
    };
    ...
    for ( integer iter{ 0 }; iter < max_iter; ++iter ) {
        ...
        for ( integer i{ 1 }; i < n; i += 2 ) {
            real_type const dK{ do_smooth( &m_clothoid_list[i - 1], &m_clothoid_list[i] ) };
        ...
        if ( m_curve_is_closed ) {
            real_type const dK{ do_smooth( &m_clothoid_list.front(), &m_clothoid_list.back() ) };
            if ( dK > max_dK ) max_dK = dK;
        }
...
bool ClothoidList::build( real_type const x0, real_type const y0, real_type const theta0,
    integer const n, real_type const s[], real_type const kappa[] )
{
    if ( n < 2 ) return false;
    ...
    UTILS_ASSERT( Utils::is_finite( k ) && Utils::is_finite( L ) && Utils::is_finite( dk ),
...
bool ClothoidList::build_raw( integer const n, real_type const x[], real_type const y[],
    real_type const abscissa[], real_type const theta[], real_type const kappa[] )
{
    if ( n < 2 ) return false;
    init();
...
ClothoidCurve const& ClothoidList::get_at_s( real_type s ) const
{
...
real_type ClothoidList::length_ISO( real_type const offs ) const
{
    real_type L{ 0 };
...
real_type ClothoidList::segment_length( integer const nseg ) const
{
    ClothoidCurve const& c{ get( nseg ) };
    return c.length();
}

real_type ClothoidList::segment_length_ISO( integer const nseg, real_type const offs ) const
{
    ClothoidCurve const& c{ get( nseg ) };
...
void ClothoidList::bb_triangles( vector<Triangle2D>& tvec, real_type const max_angle,
    real_type const max_size, integer const icurve ) const
{
...
void ClothoidList::bb_triangles_ISO( real_type const offs, vector<Triangle2D>& tvec,
    real_type const max_angle, real_type const max_size, integer const icurve ) const
{
    auto ic{ m_clothoid_list.begin() };
    for ( integer ipos{ icurve }; ic != m_clothoid_list.end(); ++ic, ++ipos )
...
void ClothoidList::bbox_ISO(
    real_type const offs, real_type& xmin, real_type& ymin, real_type& xmax, real_type& ymax ) const
{
...
void ClothoidList::evaluate(
    real_type s, real_type& th, real_type& k, real_type& x, real_type& y ) const
{
    integer const idx{ find_at_s( s ) };
    ClothoidCurve const& c{ get( idx ) };
...
void ClothoidList::evaluate_ISO( real_type s, real_type const offs, real_type& th, real_type& k,
...
real_type ClothoidList::X_ISO( real_type s, real_type const offs ) const
{
    integer const idx{ find_at_s( s ) };
    ClothoidCurve const& c{ get( idx ) };
...
real_type ClothoidList::Y_ISO( real_type s, real_type const offs ) const
{
    integer const idx{ find_at_s( s ) };
    ClothoidCurve const& c{ get( idx ) };
...
real_type ClothoidList::X_ISO_D( real_type s, real_type const offs ) const
{
    integer const idx{ find_at_s( s ) };
    ClothoidCurve const& c{ get( idx ) };
...
real_type ClothoidList::Y_ISO_D( real_type s, real_type const offs ) const
{
    integer const idx{ find_at_s( s ) };
    ClothoidCurve const& c{ get( idx ) };
...
real_type ClothoidList::X_ISO_DD( real_type s, real_type const offs ) const
{
    integer const idx{ find_at_s( s ) };
    ClothoidCurve const& c{ get( idx ) };
...
real_type ClothoidList::Y_ISO_DD( real_type s, real_type const offs ) const
{
    integer const idx{ find_at_s( s ) };
    ClothoidCurve const& c{ get( idx ) };
...
real_type ClothoidList::X_ISO_DDD( real_type s, real_type const offs ) const
{
    integer const idx{ find_at_s( s ) };
    ClothoidCurve const& c{ get( idx ) };
...
real_type ClothoidList::Y_ISO_DDD( real_type s, real_type const offs ) const
{
    integer const idx{ find_at_s( s ) };
    ClothoidCurve const& c{ get( idx ) };
...
void ClothoidList::eval_ISO( real_type s, real_type const offs, real_type& x, real_type& y ) const
{
    integer const idx{ find_at_s( s ) };
    ClothoidCurve const& c{ get( idx ) };
...
void ClothoidList::eval_ISO_D(
    real_type s, real_type const offs, real_type& x_D, real_type& y_D ) const
{
    integer const idx{ find_at_s( s ) };
    ClothoidCurve const& c{ get( idx ) };
...
void ClothoidList::eval_ISO_DD(
    real_type s, real_type const offs, real_type& x_DD, real_type& y_DD ) const
{
    integer const idx{ find_at_s( s ) };
    ClothoidCurve const& c{ get( idx ) };
...
void ClothoidList::eval_ISO_DDD(
    real_type s, real_type const offs, real_type& x_DDD, real_type& y_DDD ) const
{
    integer const idx{ find_at_s( s ) };
    ClothoidCurve const& c{ get( idx ) };
...
void ClothoidList::translate( real_type const tx, real_type const ty )
{
...
void ClothoidList::rotate( real_type const angle, real_type const cx, real_type const cy )
{
...
void ClothoidList::scale( real_type const sfactor )
{
    auto ic = m_clothoid_list.begin();
...
void ClothoidList::trim( real_type s_begin, real_type s_end, ClothoidList& newCL ) const
{
    ...
    integer const i_end{ find_at_s( s_end ) };

    if ( s_begin < s_end ) {
        // get initial and final segment
        if ( i_begin == i_end ) { // stesso segmento
            real_type const ss0{ m_s0[i_begin] };
        ...
        for ( ++i_begin; i_begin < n_seg; ++i_begin ) newCL.push_back( m_clothoid_list[i_begin] );

        for ( i_begin = 0; i_begin < i_end; ++i_begin ) newCL.push_back( m_clothoid_list[i_begin] );
...
void ClothoidList::build_AABBtree_ISO(
    real_type const offs, real_type const max_angle, real_type const max_size ) const
{
    #ifdef CLOTHOIDS_USE_THREADS
    std::lock_guard lock( m_aabb_mutex );
    #endif

    if ( m_aabb_done && Utils::is_zero( offs - m_aabb_offs ) &&
        Utils::is_zero( max_angle - m_aabb_max_angle ) &&
        Utils::is_zero( max_size - m_aabb_max_size ) )
        return;
...
bool ClothoidList::collision( BaseCurve const* pC ) const
{
...
bool ClothoidList::collision_ISO(
    real_type const offs, BaseCurve const* pC, real_type const offs_C ) const
{
    if ( pC->type() == CurveType::CLOTHOID_LIST ) {
        ClothoidList const& C{ *dynamic_cast<ClothoidList const*>( pC ) };
...
bool ClothoidList::collision_ISO(
    real_type const offs, ClothoidList const& CL, real_type const offs_CL ) const
{
    G2LIB_DEBUG_TICTOC;
    ...
    for ( const auto& [fst, snd] : intersectList ) {
        integer i = fst;
        UTILS_ASSERT_DEBUG( i >= 0 && i < static_cast<integer>( m_aabb_triangles.size() ),
        ...
        for ( integer j : snd ) {
            UTILS_ASSERT_DEBUG( j >= 0 && j < static_cast<integer>( CL.m_aabb_triangles.size() ),
...
void ClothoidList::intersect_ISO(
    real_type offs, ClothoidList const& CL, real_type offs_CL, IntersectList& ilist ) const
{
    G2LIB_DEBUG_TICTOC;
    ...
    if ( intersect_with_AABBtree ) {
        ...
        G2LIB_DEBUG_MESSAGE( "ClothoidList::intersect_ISO( offs={}, CL, offs_CL={}, ilist ) "
                             "build_AABBtree_ISO elapsed {}ms\n",
            offs, offs_CL, tictoc.elapsed_ms() );
        ...
        for ( const auto& [fst, snd] : intersectList ) {
            integer i{ fst };
            UTILS_ASSERT_DEBUG( i >= 0 && i < static_cast<integer>( m_aabb_triangles.size() ),
                "ClothoidList::intersect_ISO( offs={}, CL, offs_CL={}, ilist ) i={} out of range "
                "[0,{})\n",
                offs, offs_CL, i, m_aabb_triangles.size() );
            Triangle2D const& T1 = m_aabb_triangles.at( i );
            ClothoidCurve const& C1 = m_clothoid_list.at( T1.Icurve() );
            ...
            for ( integer j : snd ) {
                UTILS_ASSERT_DEBUG(
                    ...
                    "ClothoidList::intersect_ISO( offs={}, CL, offs_CL={}, ilist ) j={} out of "
                    "range [0,{})\n",
                    offs, offs_CL, j, CL.m_aabb_triangles.size() );
                Triangle2D const& T2 = CL.m_aabb_triangles.at( j );
                ClothoidCurve const& C2 = CL.m_clothoid_list.at( T2.Icurve() );
                ...
                bool const converged{
                    C1.aabb_intersect_ISO( T1, offs, &C2, T2, offs_CL, ss1, ss2 ) };
...
void ClothoidList::intersect( BaseCurve const* pC, IntersectList& ilist ) const
{
    if ( pC->type() == CurveType::CLOTHOID_LIST ) {
        ClothoidList const& C{ *dynamic_cast<ClothoidList const*>( pC ) };
...
void ClothoidList::intersect_ISO(
    real_type const offs, BaseCurve const* pC, real_type const offs_C, IntersectList& ilist ) const
{
    if ( pC->type() == CurveType::CLOTHOID_LIST ) {
        ClothoidList const& C{ *dynamic_cast<ClothoidList const*>( pC ) };
...
integer ClothoidList::closest_point_internal( real_type const qx, real_type const qy,
    real_type const offs, real_type& x, real_type& y, real_type& s, real_type& DST ) const
{
    G2LIB_DEBUG_TICTOC;
    ...
    G2LIB_DEBUG_MESSAGE( "ClothoidList::closest_point_internal build_AABBtree_ISO elapsed {}ms\n",
        tictoc.elapsed_ms() );
    ...
    if ( m_aabb_tree.num_tree_nodes() > G2LIB_AABB_MIN_NODES && intersect_with_AABBtree ) {
        ...
        G2LIB_DEBUG_MESSAGE( "ClothoidList::closest_point_internal min_distance_candidates elapsed "
                             "{}ms candidated #{}\n",
            tictoc.elapsed_ms(), candidateList.size() );

        UTILS_ASSERT0(
            !candidateList.empty(), "ClothoidList::closest_point_internal no candidate\n" );
        ...
        for ( Triangle2D const& T : m_aabb_triangles ) {
            ...
            if ( dst < DST ) {
                ...
                m_clothoid_list.at( T.Icurve() )
                    .closest_point_ISO( qx, qy, offs, xx, yy, ss, tt, dst );
                if ( dst < DST ) {
                    DST = dst;
                    s = ss + m_s0.at( T.Icurve() );
                    x = xx;
        ...
        G2LIB_DEBUG_TOC;
        G2LIB_DEBUG_MESSAGE( "ClothoidList::closest_point_internal minimize elapsed {}ms noAABB\n",
            tictoc.elapsed_ms() );
    }
    return icurve;
}
...
integer ClothoidList::closest_point_ISO( real_type const qx, real_type const qy,
    real_type const offs, real_type& x, real_type& y, real_type& s, real_type& t,
    real_type& DST ) const
{
    integer icurve = this->closest_point_internal( qx, qy, offs, x, y, s, DST );
    UTILS_ASSERT( icurve >= 0 && icurve < static_cast<integer>( m_clothoid_list.size() ),
    ...
    G2LIB_DEBUG_MESSAGE( "ClothoidList::closest_point_ISO\n"
                         "||P-P0|| = {} and {}, |(P-P0).T| = {}\n",
...
integer ClothoidList::closest_point_ISO( real_type const qx, real_type const qy, real_type& x,
    real_type& y, real_type& s, real_type& t, real_type& dst ) const
{
...
integer ClothoidList::closest_segment( real_type const qx, real_type const qy ) const
{
    G2LIB_DEBUG_TICTOC;
    ...
    if ( m_aabb_tree.num_tree_nodes() > G2LIB_AABB_MIN_NODES && intersect_with_AABBtree ) {
        ...
        UTILS_ASSERT0( !candidateList.empty(), "ClothoidList::closest_segment no candidate\n" );
        ...
        for ( integer const ipos : candidateList ) {
            ...
            if ( dst < DST ) {
                ...
                real_type xx, yy, ss, tt;
                m_clothoid_list.at( T.Icurve() )
                    .closest_point_ISO( qx, qy, 0, xx, yy, ss, tt, dst );
                if ( dst < DST ) {
                    DST = dst;
                    icurve = T.Icurve();
                }
        ...
        G2LIB_DEBUG_TOC;
        G2LIB_DEBUG_MESSAGE( "ClothoidList::closest_segment elapsed {}ms\n", tictoc.elapsed_ms() );
    } else {
        G2LIB_DEBUG_TIC;
        for ( Triangle2D const& T : m_aabb_triangles ) {
            real_type dst{ T.dist_min( qx, qy ) }; // distanza approssimata con triangolo
...
integer ClothoidList::closest_point_in_range_ISO( real_type const qx, real_type const qy,
    integer const icurve_begin, integer const icurve_end, real_type& x, real_type& y, real_type& s,
    real_type& t, real_type& dst, integer& icurve ) const
{
    UTILS_ASSERT0(
        !m_clothoid_list.empty(), "ClothoidList::closest_point_in_range_ISO, empty list\n" );
    integer const nsegs{ this->num_segments() };
    if ( nsegs == 1 ) { // only 1 segment to check
        icurve = 0;
        integer const res{ m_clothoid_list.front().closest_point_ISO( qx, qy, x, y, s, t, dst ) };
        s += m_s0[0];
        return res;
    }
    ...
    if ( ie < 0 ) ie += nsegs;
    UTILS_ASSERT(
        ib >= 0 && ie >= 0, "ClothoidList::closest_point_in_range_ISO, ib = {} ie = {}\n", ib, ie );

    icurve = ib;
    integer res{ m_clothoid_list.at( icurve ).closest_point_ISO( qx, qy, x, y, s, t, dst ) };
    s += m_s0.at( icurve );

    G2LIB_DEBUG_MESSAGE( "ClothoidList::closest_point_in_range_ISO\n"
    ...
    do {
        ...
        integer const C_res{
            m_clothoid_list.at( iseg ).closest_point_ISO( qx, qy, C_x, C_y, C_s, C_t, C_dst ) };
        G2LIB_DEBUG_MESSAGE(
...
integer ClothoidList::closest_point_in_s_range_ISO( real_type const qx, real_type const qy,
    real_type s_begin, real_type s_end, real_type& x, real_type& y, real_type& s, real_type& t,
    real_type& dst, integer& icurve ) const
{
    UTILS_ASSERT0(
        !m_clothoid_list.empty(), "ClothoidList::closest_point_in_s_range_ISO, empty list\n" );
    // put in range
    ...
    if ( i_begin == i_end ) {
        ...
        real_type const ss0{ m_s0[i_begin] };
        real_type const ss1{ m_s0[i_end] };
...
real_type ClothoidList::closest_point_by_sample( real_type const ds, real_type const qx,
    real_type const qy, real_type& X, real_type& Y, real_type& S ) const
{
    real_type dst{ Utils::Inf<real_type>() };
    integer i{ 0 };
    for ( ClothoidCurve const& C : m_clothoid_list ) {
        real_type xx, yy, ss;
        real_type const dd{ C.closest_point_by_sample( ds, qx, qy, xx, yy, ss ) };
        if ( dd < dst ) {
            dst = dd;
            X = xx;
            Y = yy;
...
void ClothoidList::get_SK( real_type s[], real_type kappa[] ) const
{
    auto ic = m_clothoid_list.begin();
    integer k{ 0 };
    real_type ss{ 0 };
...
void ClothoidList::get_STK( real_type s[], real_type theta[], real_type kappa[] ) const
{
    auto ic{ m_clothoid_list.begin() };
    integer k{ 0 };
    real_type ss{ 0 };
...
void ClothoidList::get_XY( real_type x[], real_type y[] ) const
{
    auto ic{ m_clothoid_list.begin() };
    integer k{ 0 };
    while ( ic != m_clothoid_list.end() ) {
...
void ClothoidList::get_delta_theta( real_type delta_theta[] ) const
{
    auto ic{ m_clothoid_list.begin() };
    integer k{ 0 };
    for ( ++ic; ic != m_clothoid_list.end(); ++ic, ++k ) {
...
void ClothoidList::get_delta_kappa( real_type deltaKappa[] ) const
{
    auto ic{ m_clothoid_list.begin() };
    integer k{ 0 };
    for ( ++ic; ic != m_clothoid_list.end(); ++ic, ++k )
...
integer ClothoidList::findST1(
    real_type const x, real_type const y, real_type& s, real_type& t ) const
{
    ...
    for ( ++ic, ++is, ++ipos; ic != m_clothoid_list.end(); ++ic, ++is, ++ipos ) {
...
integer ClothoidList::findST1( integer const ibegin, integer const iend, real_type const x,
    real_type const y, real_type& s, real_type& t ) const
{
    UTILS_ASSERT0( !m_clothoid_list.empty(), "ClothoidList::findST, empty list\n" );
    UTILS_ASSERT(
...
void ClothoidList::export_table( ostream_type& stream ) const
{
    ...
    for ( auto const& C : m_clothoid_list )
        fmt::print( stream, "{}\t{}\t{}\t{}\t{}\t{}\n", C.x_begin(), C.y_begin(), C.theta_begin(),
...
{
    for ( auto const& C : CL.m_clothoid_list ) stream << C << '\n';
    return stream;
}
...
static void save_segment( ostream_type& stream, ClothoidCurve const& c )
{
    fmt::print( stream,
        ...
        "{:<24}\t{:<24}\t{:<24}\t{:<24}\n",
        //------------------
        fmt::format( "{:.20}", c.x_begin() ), fmt::format( "{:.20}", c.y_begin() ),
        fmt::format( "{:.20}", c.theta_begin() ), fmt::format( "{:.20}", c.kappa_begin() ),
        //------------------
        fmt::format( "{:.20}", c.x_end() ), fmt::format( "{:.20}", c.y_end() ),
        fmt::format( "{:.20}", c.theta_end() ), fmt::format( "{:.20}", c.kappa_end() ) );
}
    #endif
